    ...
    if ($action === 'update_status' && $oid > 0) {
        ...
        if (in_array($newStatus, STAFF_ALLOWED_STATUSES, true)) {
            try {
                ...
                // Audit log
                logStaffActivity($pdo, 'order_status_updated', "Updated order #{$oid} status to {$newStatus}");
            } catch (\Throwable $e) {
        ...
        logStaffActivity($pdo, 'unauthorized_delete_attempt', "Attempted to delete order #{$oid} without permission");
    }
}
