...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($bid > 0 && in_array($action, ['confirm_booking', 'cancel_booking'], true)) {
        ...
        try {
            ...
            // Audit log
            logStaffActivity($pdo, 'booking_status_updated', "Updated booking #{$bid} status to {$newStatus}");
        } catch (\Throwable $e) {
        ...
        logStaffActivity($pdo, 'unauthorized_delete_attempt', "Attempted to delete booking #{$bid} without permission");
    }
}
...
<?php
$staffNavActive = 'bookings';
$staffNavFromRoot = true;
require __DIR__ . '/staffSide/staff-sidebar-nav.php';
?>
  </aside>
