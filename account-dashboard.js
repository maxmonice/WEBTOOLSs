...
(async function guardSession() {
        ...
        function initDashboard() {
            ...
            userData.phone = sessionStorage.getItem('user_phone') || '';
            userData.address = sessionStorage.getItem('user_address') || '';
        ...
        let userData = { id:'', name:'', email:'', photo:'', phone: '', address: '' };
        ...
        function updateUI() {
            ...
            document.getElementById('detailPhone').textContent   = userData.phone || 'Not set';
            document.getElementById('detailAddress').textContent = userData.address || 'Not set';
        ...
        window.saveProfile = async function() {
            ...
            const saveBtn = document.querySelector('#editModal .btn-save') || document.querySelector('#editModal .btn-logout-confirm'); 
            ...
            try {
                ...
                if (data.success) {
                    ...
                    sessionStorage.setItem('user_address', a);
...
document.getElementById('mobile-menu').addEventListener('click', () => {
        ...
        window.submitRiderRating = async function() {
            ...
            try {
                const res = await fetch('save-rating.php', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ 
                        type: 'rider', 
                ...
                const data = await res.json();
                if (data.success) {
                    showToast('Rider rated successfully!');
                    window.closeRiderRating();
                } else {
        ...
        window.submitFoodRating = async function() {
            ...
            try {
                ...
                const data = await res.json();
                if (data.success) {
                    showToast('Food rated successfully!');
                    window.ignoreFoodRating();
                    // Finalize the tracking flow
                    window.clearOrderTracking();
                } else {
                    showToast(data.message || 'Error saving rating', 'error');
                }
            } catch (e) {
        ...
        window.handlePromoSearch = async function() {
            ...
            try {
                ...
                list.innerHTML = data.promos.map(promo => {
                    let actionHtml = '';
                    if (!Number(promo.is_claimed)) {
                        actionHtml = `<button class="btn-save" onclick="claimPromo(${promo.id}, '${promo.code}')" style="padding: 12px 24px; font-size: 0.85rem; border-radius: 10px; box-shadow: 0 4px 12px rgba(194,38,38,0.25); margin-left: 15px; white-space: nowrap;">CLAIM</button>`;
                    } else if (Number(promo.is_active)) {
                        actionHtml = '<span style="color: #22c55e; font-weight: 800; font-size: 0.8rem; letter-spacing: 1px; padding: 8px 12px; background: rgba(34,197,94,0.1); border-radius: 8px; margin-left: 15px; white-space: nowrap;">ACTIVE</span>';
                    } else {
                        actionHtml = `<button class="btn-save" onclick="usePromo(${promo.id}, '${promo.code}')" style="padding: 12px 24px; font-size: 0.85rem; border-radius: 10px; background: #333; box-shadow: none; margin-left: 15px; white-space: nowrap;">USE</button>`;
                    }
        ...
        window.claimPromo = async function(id, code) {
            try {
                ...
                const data = await res.json();
                if (data.success) {
                    showToast(`Promo ${code} claimed and activated!`);
        ...
        window.usePromo = async function(id, code) {
            try {
                const res = await fetch('promo-ajax.php', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ action: 'use_promo', promo_id: id })
                });
                const data = await res.json();
                if (data.success) {
                    showToast(`Promo ${code} is now active!`);
                    localStorage.setItem('active_promo', code);
                    localStorage.setItem('promo_discount', data.discount);
                    localStorage.setItem('promo_category', data.applicable_category);
                    handlePromoSearch();
                ...
                showToast('Error activating promo.', true);
            }
        };
