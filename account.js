...
const FACEBOOK_APP_ID    = window.APP_CONFIG?.facebookAppId || '1282425887392045';
...
function signInWithGoogle() {
    ...
    // Try showing the One Tap / Select Account prompt directly
    google.accounts.id.prompt((notification) => {
        console.log('Google Prompt Notification:', notification);
        if (notification.isNotDisplayed() || notification.isSkippedMoment()) {
            // If prompt is blocked or skipped, fallback to rendering a hidden button and clicking it
            // (Standard Google practice for custom buttons)
            ...
            setTimeout(() => {
                ...
                if (btn) {
                    ...
                    showError('Google sign-in was blocked. Please check your browser settings.');
                }
            }, 300);
        }
    });
...
function loadFacebookSDK() {
    window.fbAsyncInit = function () {
        FB.init({ appId: FACEBOOK_APP_ID, cookie: true, xfbml: true, version: 'v22.0' });
    };
    const script = document.createElement('script');
...
async function callAuth(payload) {
    ...
    // Determine the silo side (customer, staff, admin)
    const urlParams = new URLSearchParams(window.location.search);
    const side = urlParams.get('side') || 'customer';
    
    const payloadWithRunId = { ...payload, runId, side };
