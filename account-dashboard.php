...
if (!isset($_SESSION['user_id']) || ($_SESSION['session_side'] ?? '') !== 'customer') {
...
<body>
        ...
        <div class="page">
            ...
            <div class="section-block">
                ...
                <div class="detail-rows">
                    ...
                    <div class="detail-row"><span class="dr-label">Phone Number</span><span class="dr-value" id="detailPhone">Not set</span></div>
                    <div class="detail-row"><span class="dr-label">Home Address</span><span class="dr-value" id="detailAddress">Not set</span></div>
            ...
            <div id="foodRatingSection" style="display:none;" class="section-block">
                <div class="section-label" style="display:flex; justify-content:space-between; align-items:center;">
                    ...
                    <button class="modal-close" onclick="ignoreFoodRating()" style="width:24px; height:24px; font-size:0.7rem; border:none; background:rgba(255,255,255,0.05);"><i class="fa-solid fa-x" style="color: rgb(0, 0, 0);"></i></button>
                </div>
    ...
    <div class="modal-overlay" id="editModal">
        <div class="modal">
            <div class="modal-head">
                ...
                <button class="modal-close" onclick="closeEdit()"><i class="fa-solid fa-x" style="color: rgb(0, 0, 0);"></i></button>
            </div>
            <div class="modal-body">
                ...
                <div class="field-group"><label>Phone Number</label><input type="text" id="inputPhone" placeholder="0912 345 6789" /></div>
                <div class="field-group"><label>Home Address</label><textarea id="inputAddress" placeholder="Street, City, Zip" style="width:100%; min-height:80px; background:var(--surface2); border:1px solid rgba(255,255,255,0.08); border-radius:8px; padding:12px; color:#fff; font-family:inherit; resize:vertical;"></textarea></div>
            </div>
    ...
    <div class="modal-overlay" id="changePwModal">
        <div class="modal">
            <div class="modal-head">
                ...
                <button class="modal-close" onclick="closeChangePw()"><i class="fa-solid fa-x" style="color: rgb(0, 0, 0);"></i></button>
            </div>
            <div class="modal-body">
    ...
    <div class="modal-overlay" id="logoutModal">
        <div class="modal">
            <div class="modal-head">
                <h3>Log Out</h3>
                <button class="modal-close" onclick="closeLogout()"><i class="fa-solid fa-x" style="color: rgb(0, 0, 0);"></i></button>
            </div>
    ...
    <div class="modal-overlay" id="bookingDetailModal">
        <div class="modal" style="max-width:560px;">
            <div class="modal-head">
                ...
                <button class="modal-close" onclick="closeBookingDetail()"><i class="fa-solid fa-x" style="color: rgb(0, 0, 0);"></i></button>
            </div>
    ...
    <div class="map-fullscreen-overlay" id="bookingDetailFullscreen" style="z-index:10002;">
        <div class="map-fs-header">
            ...
            <button class="map-fs-close" onclick="closeBookingDetail()"><i class="fa-solid fa-x" style="color: rgb(0, 0, 0);" style="color:#fff;"></i></button>
        </div>
    ...
    <div class="map-fullscreen-overlay" id="promoFullscreen" style="z-index:10001; background:#0a0a0a;">
        <div class="map-fs-header" style="border-bottom:1px solid rgba(255,255,255,0.05);">
            ...
            <button class="map-fs-close" onclick="closePromos()"><i class="fa-solid fa-x" style="color: rgb(0, 0, 0);"></i></button>
        </div>
    ...
    <div class="map-fullscreen-overlay" id="historyFullscreen" style="z-index:10001; background:#0a0a0a;">
        <div class="map-fs-header" style="border-bottom:1px solid rgba(255,255,255,0.05);">
            ...
            <button class="map-fs-close" onclick="closeHistory()"><i class="fa-solid fa-x" style="color: rgb(0, 0, 0);"></i></button>
        </div>
    ...
    <div class="map-fullscreen-overlay" id="mapFullscreen" style="z-index:10001;">
        <div class="map-fs-header">
            ...
            <button class="map-fs-close" onclick="closeMapFullscreen()"><i class="fa-solid fa-x" style="color: rgb(0, 0, 0);"></i></button>
        </div>
    ...
    <div class="modal-overlay rating-modal-overlay" id="riderRatingModal">
        <div class="modal">
            <div class="modal-head" style="padding-bottom:10px;">
                ...
                <button class="modal-close" onclick="closeRiderRating()"><i class="fa-solid fa-x" style="color: rgb(0, 0, 0);"></i></button>
            </div>
    ...
    <div class="modal-overlay" id="cancelOrderModal">
        <div class="modal">
            <div class="modal-head">
                <h3>Cancel Order</h3>
                <button class="modal-close" onclick="closeCancelModal()"><i class="fa-solid fa-x" style="color: rgb(0, 0, 0);"></i></button>
            </div>
            <div class="logout-confirm-body">
    ...
    <div class="modal-overlay" id="chatModal" style="z-index:10010;">
        <div class="modal" style="width:100%; max-width:400px; height:80vh; display:flex; flex-direction:column; padding:0; overflow:hidden; background:#0d0204;">
            <div class="modal-head" style="padding:15px 20px; background:var(--red-dark); border:none;">
                ...
                <button class="modal-close" onclick="closeChatModal()"><i class="fa-solid fa-x" style="color: rgb(0, 0, 0);"></i></button>
            </div>
    ...
    <div class="modal-overlay" id="cancelBookingConfirmModal" style="z-index:10005;">
        <div class="modal" style="background:rgba(18,18,18,0.95); backdrop-filter:blur(15px); border:1px solid rgba(255,255,255,0.08); box-shadow:0 25px 50px rgba(0,0,0,0.5);">
            <div class="modal-head" style="border-bottom:1px solid rgba(255,255,255,0.05); padding:20px 25px;">
                ...
                <button class="modal-close" onclick="closeCancelBookingModal()"><i class="fa-solid fa-x" style="color: rgb(0, 0, 0);"></i></button>
            </div>
    ...
    <script>
        ...
        async function saveProfile() {
            ...
            try {
                const res = await fetch('update-profile.php', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ name: n, email: e, phone: p, address: a })
                });
                const data = await res.json();
                if (data.success) {
                    userData.name = n; userData.email = e; userData.phone = p; userData.address = a;
                    sessionStorage.setItem('user_name', n); sessionStorage.setItem('user_email', e);
                    sessionStorage.setItem('user_phone', p); sessionStorage.setItem('user_address', a);
