    ...
    for ($day = 1; $day <= $daysInMonth; $day++) {
        ...
        if (isset($bookingsByDate[$day])) {
            ...
            foreach ($bookingsByDate[$day] as $booking) {
                ...
                $calendar .= '<div onclick="event.stopPropagation(); showBookingDetails(' . $booking['id'] . ')">' . $bookingId . ' ' . $customerName . '</div>';

                

                // Add action buttons for pending and confirmed bookings

                if ($booking['status'] === 'pending') {

                    $calendar .= '<div class="cal-event-actions">';

                    $calendar .= '<button class="cal-action-btn confirm" title="Confirm" onclick="event.stopPropagation(); updateBookingStatus(' . $booking['id'] . ', \'confirmed\')"><i class="fa-solid fa-check"></i></button>';

                    $calendar .= '<button class="cal-action-btn cancel" title="Cancel" onclick="event.stopPropagation(); updateBookingStatus(' . $booking['id'] . ', \'cancelled\')"><i class="fa-solid fa-xmark"></i></button>';
                          ...
                          <td>
                            ...
                            <div class="booking-actions">
                              ...
                              <?php if ($booking['status'] === 'pending'): ?>
                                ...
                                <button class="btn btn-success btn-xs" title="Confirm" onclick="updateBookingStatus(<?= $booking['id'] ?>, 'confirmed')">

                                  <i class="fa-solid fa-check"></i> Confirm

                                </button>

                                <button class="btn btn-danger btn-xs" title="Cancel" onclick="updateBookingStatus(<?= $booking['id'] ?>, 'cancelled')">
